...
def get_reviews_and_retention(start_date, end_date):
    ...
    true_reviews = mw.col.db.scalar("SELECT COUNT(*) FROM revlog WHERE id >= ? AND id < ? AND type <> 0 AND time != 0", start, end) 
    true_flunked = mw.col.db.scalar("SELECT COUNT(*) FROM revlog WHERE ease == 1 AND id >= ? AND id < ? AND type <> 0 AND time != 0", start, end) 
